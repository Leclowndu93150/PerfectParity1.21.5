...
public class SheepColorSpawnRules {
    ...
    private static SheepColorSpawnConfiguration getSheepColorConfiguration(Holder<Biome> holder) {
        if (holder.is(ParityTags.SPAWNS_WARM_VARIANT_FARM_ANIMALS)) {
            return WARM_SPAWN_CONFIGURATION;
        } else {
            return holder.is(ParityTags.SPAWNS_COLD_VARIANT_FARM_ANIMALS) ? COLD_SPAWN_CONFIGURATION : TEMPERATE_SPAWN_CONFIGURATION;
        }
    }
