...
import com.leclowndu93150.perfectparity.entity.utils.ParityTags;
...
public class AmbientDesertBlockSoundsPlayer {
    ...
    public static void playAmbientDeadBushSounds(Level level, BlockPos blockPos, RandomSource randomSource) {
        if (randomSource.nextInt(130) == 0) {
            ...
            if (AmbientDesertBlockSoundsPlayer.shouldPlayDesertDryVegetationBlockSounds(level, blockPos.below())) {
                level.playLocalSound(blockPos.getX(), blockPos.getY(), blockPos.getZ(), ModSounds.DEAD_BUSH_IDLE.get(), SoundSource.AMBIENT, 1.0f, 1.0f, false);
            }
        }
    }

    public static boolean shouldPlayDesertDryVegetationBlockSounds(Level level, BlockPos blockPos) {
        return level.getBlockState(blockPos).is(ParityTags.TRIGGERS_AMBIENT_DESERT_DRY_VEGETATION_BLOCK_SOUNDS) && level.getBlockState(blockPos.below()).is(ParityTags.TRIGGERS_AMBIENT_DESERT_DRY_VEGETATION_BLOCK_SOUNDS);
    }
    ...
    private static boolean canTriggerAmbientDesertSandSounds(BlockState blockState) {
        return blockState.is(ParityTags.TRIGGERS_AMBIENT_DESERT_SAND_BLOCK_SOUNDS);
    }
}
